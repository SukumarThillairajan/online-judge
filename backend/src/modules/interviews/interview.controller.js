...
export const streamInterviewChat = async (req, res) => {
    try {
        const { sessionId, problemId, currentCode, message } = req.body;
        ...
        const stream = await getInterviewerStream(chatHistory, problem, currentCode);
...
export const finishInterviewAndGrade = async (req, res) => {
    try {
        const { sessionId, submissionId, problemId, finalCode } = req.body;
        if (!sessionId || !submissionId || !problemId || typeof finalCode === 'undefined') {
        ...
        const aiEvaluation = await evaluateInterview(chatHistory, problem, finalCode);
        ...
        return res.status(200).json({
            success: true,
            rank: gamifiedRank,
            breakdown: aiEvaluation.score_breakdown
        });
    }
    catch (error) {
