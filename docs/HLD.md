...
*Version 2.0 — AI Interviewer Integration, updated with session resilience (server-authoritative timer, resumable sessions on reload including the candidate's chat history and code editor state) and adaptive interview pacing. This document supersedes the V1 HLD and describes the system as it is currently deployed.*
...
| /api/interviews/code | POST | Autosaves the candidate's live editor contents (code + language) to Redis on a debounce, so a reload restores exactly what they were typing (§6.3). Not rate limited — it's a cheap cache write, not an AI call. |
